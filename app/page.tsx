...
export default function BookingPage() {
  ...
  // FUNGSI HANTAR WHATSAPP DENGAN LINK QR CODE
  const sendToWhatsApp = () => {
    // Kita gunakan API QR Server untuk jana link gambar QR secara dinamik
    const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${bookingRef}`;
    
    const message = `*TEMPAHAN NASI AYAM HAJI ALI*%0A` +
                    `--------------------------%0A` +
                    `Ref: *${bookingRef}*%0A` +
                    `Nama: ${form.name}%0A` +
                    `Cawangan: ${form.branchName}%0A` +
                    `Tarikh: ${form.date}%0A` +
                    `Masa: ${form.time}%0A` +
                    `Tetamu: ${form.guests} Pax%0A` +
                    `--------------------------%0A` +
                    `*LIHAT TIKET QR DI SINI:*%0A${qrImageUrl}%0A` +
                    `--------------------------%0A` +
