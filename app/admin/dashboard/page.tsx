...
import { 
  ...
  BarChart3, Loader2, Edit3, Save, X, TrendingUp
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleAddStaff = async () => {
    ...
    if (result.success) { alert("Staf Ditambah!"); window.location.reload(); }
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row font-sans">
      ...
      <aside className="w-full md:w-72 bg-white border-r p-8 flex flex-col shadow-sm sticky top-0 h-screen z-50">
        ...
        <nav className="flex-grow space-y-2">
          ...
          {profile.role === 'master' && (
            <>
              <div className="pt-4 pb-2 text-[10px] font-black text-slate-300 uppercase tracking-widest ml-4">HQ Tools</div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                        <p className="text-xs font-black text-slate-300 uppercase tracking-widest">Total Bookings</p>
                        <p className="text-4xl font-black text-slate-800 mt-2">{bookings.length}</p>
                    </div>
                    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                        <p className="text-xs font-black text-slate-300 uppercase tracking-widest">Arrived Today</p>
                        <p className="text-4xl font-black text-green-500 mt-2">{bookings.filter(b => b.status === 'checked_in').length}</p>
                    </div>
                    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                        <p className="text-xs font-black text-slate-300 uppercase tracking-widest">Growth</p>
                        <p className="text-4xl font-black text-blue-500 mt-2">+12%</p>
                    </div>
                </div>
                <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 h-64 flex items-center justify-center text-slate-300 font-bold uppercase tracking-widest italic">
                    <TrendingUp className="mr-2"/> Graph View Coming Soon
                </div>
            </motion.div>
          )}
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {bookings.map((b) => (
                            <tr key={b.id} className="hover:bg-slate-50 transition-all">
                                <td className="p-6 font-bold uppercase text-sm text-slate-700">{b.customer_name} <br/> <span className="text-[10px] text-slate-300">+{b.customer_phone}</span></td>
                                <td className="p-6 text-xs font-black text-slate-400 uppercase italic">{b.branches?.name}</td>
                                <td className="p-6 text-xs font-bold text-slate-600">{b.booking_date} | {b.booking_time}</td>
                                <td className="p-6">
                                    {b.status === 'confirmed' ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 space-y-6 h-fit">
                    <h3 className="font-black uppercase italic text-sm flex items-center gap-2 tracking-tight">
                        <div className="bg-red-50 p-2 rounded-lg text-red-600"><UserPlus size={18}/></div> {editingStaff ? 'Kemaskini Staf' : 'Daftar Staf'}
                    </h3>
                ...
                <div className="lg:col-span-2 space-y-4">
                    {staffList.map(st => (
                        <div key={st.id} className="bg-white p-6 rounded-[35px] shadow-sm border border-slate-50 flex justify-between items-center group">
